...
const EditClient = (props) => {
  ...
  const editHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://localhost:7126/api/v1/user/${editUserData.id}`,
        editUserData
      );
    } catch (error) {
  ...
  return (
    <form className="flex flex-col md:flex-wrap space-y-2 bg-semi px-24 py-12 rounded-xl">
      <AiOutlineCloseCircle
        onClick={props.onClick}
        className="cursor-pointer text-2xl absolute top-[170px] right-[310px]"
      />
      <Input
        diff="id"
        title="Id"
